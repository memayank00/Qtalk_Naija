...
/**endpoints */
import { getAllUsersList, sendFriendRequest } from "../../utils/endpoints";
/**actions */
...
class AddFriendSearch extends Component {
      ...
      <div className="App">
        <div className="findFrdBg">
          <div className="container">
            <div className="row">
              <div className="col-md-4 col-xs-12 findfrdHd">Find Friend</div>
  ...
  friendList(params = {}) {
    this.setState({ isLoading: true });
      HTTP.Request("get", getAllUsersList, params)
      .then(response => {
  ...
  addFrnd(data) {
    ...
    HTTP.Request("post", sendFriendRequest, obj)
      .then(response => {
        if (response.type === "error") {
          toast(response.errors, { type: "info" });
        } else {
          let socketData={ message:`${user.name} sent you a friend request.`,
                           type: 'Friend' , to: data}
...
export default connect(mapStatesToProps)(AddFriendSearch);
