    ...
    render() {
        ...
        return (
            <tr className="odd gradeX" >
             <td width="15%"> {cms.custom_link} </td>
                <td  width="25%"> {cms.title} </td>              
               {/*  <td className="center text-center"> {cms.image ? <img className='img-thumbnail' src={cms.image.secure_url} width="120" alt="slide" /> : <label className="label label-danger" >No Image </label>} </td> */}
                ...
                <td width="15%"><Moment format="DD MMM YYYY">{cms.created_at}</Moment></td>
                <td className="center" width="10%"> <label className={cms.status?'label label-info':'label label-danger'}>{cms.status?"Active":"Pending"}</label> </td>
                <td width="10%">
                   {/*  <TT tooltip="Remove"><a className="btn btn-xs red-mint" onClick={this.showModal}><i className="fa fa-trash no-pointer"></i></a></TT> */}
                    <TT tooltip="Edit"><Link to={'/cms-blocks/edit/' + cms._id} className="btn btn-xs grey-mint" ><i className="fa fa-pencil-square-o"></i></Link></TT>
