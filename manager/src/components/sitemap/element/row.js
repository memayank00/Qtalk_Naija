    ...
    render() {
        ...
        return (
            <tr className="odd gradeX" >
                <td width="40%"> {cms.link} </td>
               {/*  <td className="center"> {cms.order} </td> */}
                <td width="10%"><Moment format="DD MMM YYYY">{cms.created_at}</Moment></td>
                <td width="10%" className="center"> <label className={cms.status?'label label-info':'label label-danger'}>{cms.status?"Active":"IN-ACTIVE"}</label> </td>
                <td width="10%">                   
                    
                    <TT tooltip="Remove"><a className=" btn btn-xs red-mint" onClick={this.showModal}><i className="fa fa-trash no-pointer"  ></i></a></TT>
                    <TT tooltip="Edit"><Link to={'/sitemap/edit/' + cms._id} className=" btn btn-xs grey-mint"><i className="fa fa-pencil-square-o" ></i></Link></TT>
